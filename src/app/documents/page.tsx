...
export default function DocumentsPage() {
  ...
  if (viewingDoc) {
    return (
      <div className="fixed inset-0 z-[100] bg-white dark:bg-[#0a0b1e] flex flex-col animate-in fade-in">
        {/* Viewer Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-3 bg-white dark:bg-[#11152a] border-b border-gray-200 dark:border-white/5 flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <button
              onClick={() => setViewingDoc(null)}
              className="flex items-center gap-2 text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors p-2 rounded-lg hover:bg-white/5 flex-shrink-0"
            >
            ...
            <div className="min-w-0">
              <h2 className="text-gray-900 dark:text-white font-bold text-sm truncate">{viewingDoc.title}</h2>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <button
              onClick={() => setViewingDoc(null)}
              className="p-2 rounded-lg text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-white/5 transition-colors"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-in pb-20">
      ...
      <div className="relative overflow-hidden rounded-2xl sm:rounded-3xl bg-white dark:bg-[#11152a] p-5 sm:p-8 border border-accent-purple/20 shadow-2xl">
        ...
        <div className="relative z-10">
          ...
          <h1 className="text-2xl sm:text-5xl font-black text-gray-900 dark:text-white mb-3 italic tracking-tighter">
            PDF <span className="text-accent-purple-glow">DOCUMENTS</span>
          </h1>
          <p className="text-gray-500 dark:text-gray-400 max-w-lg leading-relaxed text-xs sm:text-sm">
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          <div className="w-20 h-20 rounded-2xl bg-gray-100 dark:bg-white/5 flex items-center justify-center mx-auto mb-4">
            <svg className="w-10 h-10 text-gray-500 dark:text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
          ...
          <p className="text-gray-500 dark:text-gray-600 text-xs mt-2">Try adjusting your filters or check back later.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((doc) => (
            <button
              ...
              className="glass-card p-5 text-left flex flex-col gap-3 group hover:border-accent-purple/40 hover:shadow-lg hover:shadow-purple-500/5 transition-all border-gray-200 dark:border-white/5 relative overflow-hidden"
            >
              ...
              <div className="relative z-10 flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-gray-900 dark:text-white text-sm font-bold leading-relaxed line-clamp-2 group-hover:text-accent-purple-light transition-colors">
                    {doc.title}
                  </p>
              ...
              <div className="relative z-10 flex flex-wrap items-center gap-2 mt-auto pt-2">
                <span className="text-[10px] px-2 py-1 rounded-full bg-gray-200 dark:bg-dark-500 text-gray-500 dark:text-gray-400 font-bold uppercase">
                  {doc.major || 'Both'}
                </span>
                ...
                <span className="text-[10px] text-gray-500 dark:text-gray-600 font-bold ml-auto">
                  {formatFileSize(doc.file_size)}
                </span>
              </div>

              <div className="relative z-10 flex items-center justify-between pt-2 border-t border-gray-200 dark:border-white/5 mt-1">
