      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-surface-container-lowest border-b border-outline-variant z-[60] px-4 flex items-center justify-between">
        ...
        >
          <div className="space-y-1.5">
            <div className={`h-0.5 w-5 bg-current rounded-full transition-all duration-300 ${isMobileOpen ? 'rotate-45 translate-y-[5px]' : ''}`} />
            <div className={`h-0.5 bg-current rounded-full transition-all duration-300 ${isMobileOpen ? 'opacity-0 w-0' : 'w-4'}`} />
            <div className={`h-0.5 w-5 bg-current rounded-full transition-all duration-300 ${isMobileOpen ? '-rotate-45 -translate-y-[5px]' : ''}`} />
          </div>
        </button>
      </header>
